...
document.getElementById("correspondenciaForm").addEventListener("submit", function (e) {
  ...
  doc.rect(10, 190, 190, 30); // Ampliado a 50 para incluir instructivo
  doc.text("QUINTO DESTINATARIO:", 12, 195);
  doc.setFont("helvetica", "normal");
  doc.text(`${destinatarioNombre} - ${cargoDestinatario}`, 65, 195);
  doc.setFont("helvetica", "bold");
  doc.text("INSTRUCTIVO:", 12, 203);
  doc.setFont("helvetica", "normal");
  const instructivoTexto = doc.splitTextToSize(instructivo, 185);
  doc.text(instructivoTexto, 12, 208);
